...
> **已知局限：** 自然图片本身有丰富的频率分量，半透明平铺水印的频域峰值容易被淹没。实施时需要用真实水印图片验证效果，如果准确率低于预期，可将 FFT 检测降级为辅助提示（提示用户使用手动涂抹），不作为主要检测手段。
...
- 使用离屏 canvas（offscreen canvas）单独记录涂抹轨迹，与原图预览分离
- 导出遮罩时只读取离屏 canvas 内容，不依赖颜色区分，涂抹区域为白色、其余黑色
